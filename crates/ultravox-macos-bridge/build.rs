...
fn main() {
    ...
    let swift_package_dir = manifest_dir.join("native").join("UltraVoxMacOSBridge");
    ...
    let status = Command::new("swift")
        ...
        .status()
        .expect("failed to run `swift build` for UltraVoxMacOSBridge");

    if !status.success() {
        panic!("`swift build` failed for UltraVoxMacOSBridge");
    }

    // SwiftPM places static libraries under .build/<config>.
    let lib_dir = swift_package_dir.join(".build").join(&swift_build_config);

    let lib_name = "libUltraVoxMacOSBridge.a";
    ...
    println!("cargo:rustc-link-lib=static=UltraVoxMacOSBridge");
