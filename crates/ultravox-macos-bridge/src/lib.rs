...
pub fn start_modifier_hotkey(modifier: &str) -> i32 {
    unsafe {
        CString::new(modifier).map_or(-1, |value| {
            ultravox_macos_bridge_start_modifier_hotkey(value.as_ptr())
        })
    }
}
...
pub fn start_key_combination_hotkey(combo: &str, hold_to_record: bool) -> i32 {
    unsafe {
        CString::new(combo).map_or(-1, |value| {
            ultravox_macos_bridge_start_key_combination_hotkey(
                value.as_ptr(),
...
pub fn set_indicator_state(state: &str) -> i32 {
    unsafe {
        CString::new(state).map_or(-1, |value| {
            ultravox_macos_bridge_set_indicator_state(value.as_ptr())
        })
    }
}
...
pub fn cancel_transcription(recording_id: &str) -> bool {
    unsafe {
        CString::new(recording_id).map_or(false, |value| {
            ultravox_macos_bridge_cancel_transcription(value.as_ptr()) != 0
        })
    }
}

pub fn prepare_model(version: &str, directory: Option<&std::path::Path>) -> bool {
    call_model(version, directory, |version, directory| unsafe {
        ultravox_macos_bridge_prepare_model(version, directory) != 0
    })
}

pub fn is_model_downloaded(version: &str, directory: Option<&std::path::Path>) -> bool {
    call_model(version, directory, |version, directory| unsafe {
        ultravox_macos_bridge_is_model_downloaded(version, directory) != 0
    })
}
...
fn call_model(
    version: &str,
    directory: Option<&std::path::Path>,
    call: impl FnOnce(*const c_char, *const c_char) -> bool,
) -> bool {
...
pub fn model_progress(version: &str) -> f64 {
    unsafe {
        CString::new(version).map_or(0.0, |value| {
            ultravox_macos_bridge_get_model_progress(value.as_ptr()).clamp(0.0, 1.0)
        })
    }
}
