...
//! `native/DictatorMacOSBridge/Sources/DictatorMacOSBridge`.
//!
...
#[allow(dead_code)]
extern "C" {
    fn dictator_macos_bridge_version() -> *mut c_char;
    fn dictator_macos_bridge_free_string(s: *mut c_char);

    fn dictator_macos_bridge_is_accessibility_trusted(prompt: c_int) -> c_int;
    fn dictator_macos_bridge_microphone_authorization_status() -> c_int;
    fn dictator_macos_bridge_request_microphone_access() -> c_int;
    fn dictator_macos_bridge_start_meeting_capture(
        path: *const c_char,
        error: *mut *mut c_char,
    ) -> c_int;
    fn dictator_macos_bridge_stop_meeting_capture(
        output_path: *mut *mut c_char,
        error: *mut *mut c_char,
    ) -> c_int;
    fn dictator_macos_bridge_set_meeting_capture_failure_callback(
        callback: extern "C" fn(error: *const c_char),
    );
    fn dictator_macos_bridge_get_caret_position(x: *mut c_double, y: *mut c_double) -> c_int;
    fn dictator_macos_bridge_capture_insertion_target(x: *mut c_double, y: *mut c_double) -> c_int;
    fn dictator_macos_bridge_clear_insertion_target();
    fn dictator_macos_bridge_paste_text(text: *const c_char) -> c_int;

    fn dictator_macos_bridge_start_modifier_hotkey(modifier: *const c_char) -> c_int;
    fn dictator_macos_bridge_stop_modifier_hotkey() -> c_int;

    fn dictator_macos_bridge_start_key_combination_hotkey(
        combo: *const c_char,
        hold_to_record: c_int,
    ) -> c_int;
    fn dictator_macos_bridge_stop_key_combination_hotkey() -> c_int;
    fn dictator_macos_bridge_set_key_combination_callback(
        callback: extern "C" fn(event: c_int, combo: *const c_char),
    );
    fn dictator_macos_bridge_start_meeting_hotkey(combo: *const c_char) -> c_int;
    fn dictator_macos_bridge_stop_meeting_hotkey() -> c_int;
    fn dictator_macos_bridge_set_meeting_hotkey_callback(
        callback: extern "C" fn(event: c_int, combo: *const c_char),
    );

    fn dictator_macos_bridge_show_indicator(x: c_double, y: c_double) -> c_int;
    fn dictator_macos_bridge_set_indicator_state(state: *const c_char) -> c_int;
    fn dictator_macos_bridge_hide_indicator() -> c_int;

    fn dictator_macos_bridge_transcribe_file(path: *const c_char, text: *mut *mut c_char) -> c_int;
    fn dictator_macos_bridge_transcribe_file_with_version(
        path: *const c_char,
    ...
    fn dictator_macos_bridge_cancel_transcription(recording_id: *const c_char) -> c_int;
    fn dictator_macos_bridge_prepare_model(
        version: *const c_char,
        directory: *const c_char,
    ) -> c_int;
    fn dictator_macos_bridge_is_model_downloaded(
        version: *const c_char,
        directory: *const c_char,
    ) -> c_int;
    fn dictator_macos_bridge_get_model_progress(version: *const c_char) -> c_double;
}

/// Returns the bridge version string.
pub fn version() -> String {
    unsafe {
        let ptr = dictator_macos_bridge_version();
        let s = c_char_to_string(ptr);
        dictator_macos_bridge_free_string(ptr);
        s
    }
}
...
pub fn microphone_authorization_status() -> MicrophoneAuthorizationStatus {
    match unsafe { dictator_macos_bridge_microphone_authorization_status() } {
...
pub fn request_microphone_access() -> bool {
    unsafe { dictator_macos_bridge_request_microphone_access() != 0 }
}
...
pub fn start_meeting_capture(path: &std::path::Path) -> Result<(), String> {
    ...
    let result = unsafe { dictator_macos_bridge_start_meeting_capture(path.as_ptr(), &mut error) };
...
pub fn stop_meeting_capture() -> Result<std::path::PathBuf, String> {
    ...
    let mut error = std::ptr::null_mut();
    let result =
        unsafe { dictator_macos_bridge_stop_meeting_capture(&mut output_path, &mut error) };
...
pub fn set_meeting_capture_failure_callback(callback: extern "C" fn(error: *const c_char)) {
    unsafe { dictator_macos_bridge_set_meeting_capture_failure_callback(callback) }
}
...
pub fn is_accessibility_trusted(prompt: bool) -> bool {
    unsafe { dictator_macos_bridge_is_accessibility_trusted(prompt as c_int) != 0 }
}
...
pub fn get_caret_position() -> (f64, f64, i32) {
    unsafe {
        ...
        let found = dictator_macos_bridge_get_caret_position(&mut x, &mut y);
        (x, y, found)
    }
}
...
pub fn capture_insertion_target() -> (f64, f64, bool) {
    unsafe {
        let mut x: c_double = 0.0;
        let mut y: c_double = 0.0;
        let has_target = dictator_macos_bridge_capture_insertion_target(&mut x, &mut y) != 0;
        (x, y, has_target)
    }
}
...
pub fn clear_insertion_target() {
    unsafe { dictator_macos_bridge_clear_insertion_target() }
}
...
pub fn paste_text(text: &str) -> i32 {
    unsafe {
        match CString::new(text) {
            Ok(cstr) => dictator_macos_bridge_paste_text(cstr.as_ptr()),
            Err(_) => -1,
        }
    }
}
...
pub fn start_modifier_hotkey(modifier: &str) -> i32 {
    unsafe {
        match CString::new(modifier) {
            Ok(cstr) => dictator_macos_bridge_start_modifier_hotkey(cstr.as_ptr()),
            Err(_) => -1,
        }
    }
}
...
pub fn stop_modifier_hotkey() -> i32 {
    unsafe { dictator_macos_bridge_stop_modifier_hotkey() }
}
...
pub fn start_key_combination_hotkey(combo: &str, hold_to_record: bool) -> i32 {
    unsafe {
        match CString::new(combo) {
            Ok(cstr) => dictator_macos_bridge_start_key_combination_hotkey(
                cstr.as_ptr(),
...
pub fn stop_key_combination_hotkey() -> i32 {
    unsafe { dictator_macos_bridge_stop_key_combination_hotkey() }
}
...
pub fn set_key_combination_callback(callback: extern "C" fn(event: c_int, combo: *const c_char)) {
    unsafe { dictator_macos_bridge_set_key_combination_callback(callback) }
}
...
pub fn start_meeting_hotkey(combo: &str) -> i32 {
    unsafe {
        match CString::new(combo) {
            Ok(cstr) => dictator_macos_bridge_start_meeting_hotkey(cstr.as_ptr()),
            Err(_) => -1,
        }
    }
}
...
pub fn stop_meeting_hotkey() -> i32 {
    unsafe { dictator_macos_bridge_stop_meeting_hotkey() }
}
...
pub fn set_meeting_hotkey_callback(callback: extern "C" fn(event: c_int, combo: *const c_char)) {
    unsafe { dictator_macos_bridge_set_meeting_hotkey_callback(callback) }
}
...
pub fn show_indicator(x: f64, y: f64) -> i32 {
    unsafe { dictator_macos_bridge_show_indicator(x, y) }
}
...
pub fn set_indicator_state(state: &str) -> i32 {
    unsafe {
        match CString::new(state) {
            Ok(cstr) => dictator_macos_bridge_set_indicator_state(cstr.as_ptr()),
            Err(_) => -1,
        }
    }
}
...
pub fn hide_indicator() -> i32 {
    unsafe { dictator_macos_bridge_hide_indicator() }
}
...
pub fn transcribe_file_with_version_for_recording_in_directory(
    ...
    unsafe {
        ...
        let success = dictator_macos_bridge_transcribe_file_with_version(
            cstr.as_ptr(),
            version_cstr.as_ptr(),
        ...
        dictator_macos_bridge_free_string(text);
        if success != 0 {
            Ok(result)
        } else {
            Err(())
...
pub fn cancel_transcription(recording_id: &str) -> bool {
    unsafe {
        match CString::new(recording_id) {
            Ok(cstr) => dictator_macos_bridge_cancel_transcription(cstr.as_ptr()) != 0,
            Err(_) => false,
        }
    }
}
...
pub fn prepare_model(version: &str, directory: Option<&std::path::Path>) -> bool {
    ...
    unsafe {
        dictator_macos_bridge_prepare_model(
            version.as_ptr(),
            directory
                .as_ref()
                .map_or(std::ptr::null(), |path| path.as_ptr()),
...
pub fn is_model_downloaded(version: &str, directory: Option<&std::path::Path>) -> bool {
    ...
    let directory = directory.and_then(|path| CString::new(path.to_string_lossy().as_bytes()).ok());
    unsafe {
        dictator_macos_bridge_is_model_downloaded(
            version.as_ptr(),
            directory
                .as_ref()
                .map_or(std::ptr::null(), |path| path.as_ptr()),
...
pub fn model_progress(version: &str) -> f64 {
    unsafe {
        CString::new(version)
            .map(|value| dictator_macos_bridge_get_model_progress(value.as_ptr()))
            .unwrap_or(0.0)
    }
}
...
fn take_bridge_string(ptr: *mut c_char) -> String {
    ...
    unsafe { dictator_macos_bridge_free_string(ptr) };
    value
}
