...
class OnboardingViewModel: ObservableObject {
    ...
    @Published var modelChoices: [OnboardingModelChoice] = []
    ...
    init() {
        ...
        initializeModelChoices()
    }

    func initializeModelChoices() {
        modelChoices = OnboardingModelChoices.availableModels.map { model in
        ...
        if selectedModelId == nil {
            selectedModelId = modelChoices.first?.id
        }
    }
    ...
    var canContinue: Bool {
        ...
        return modelChoices.contains { $0.id == selectedId && $0.isDownloaded }
    }
    
    func selectModel(_ model: OnboardingModelChoice) {
        selectedModelId = model.id
        AppPreferences.shared.selectedEngine = "fluidaudio"
        AppPreferences.shared.fluidAudioModelVersion = model.version
    }

    @MainActor
    func downloadModel(_ model: OnboardingModelChoice) async throws {
        ...
        if let index = modelChoices.firstIndex(where: { $0.id == model.id }) {
            modelChoices[index].downloadProgress = 0.0
        }
        
        try await downloadFluidAudioModel(model: model, version: model.version)
    }
    
    @MainActor
    private func downloadFluidAudioModel(model: OnboardingModelChoice, version: String) async throws {
            ...
            do {
                ...
                guard !Task.isCancelled else {
                    await MainActor.run {
                        ...
                        if let index = self.modelChoices.firstIndex(where: { $0.id == model.id }) {
                            self.modelChoices[index].downloadProgress = 0.0
                        }
                    }
                ...
                await MainActor.run {
                    if let index = modelChoices.firstIndex(where: { $0.id == model.id }) {
                        modelChoices[index].isDownloaded = true
                        modelChoices[index].downloadProgress = 1.0
                    }
                    selectModel(model)
                    isDownloading = false
    ...
    func cancelDownload() {
        ...
        if let modelName = downloadingModelName {
            if let index = modelChoices.firstIndex(where: { $0.name == modelName }) {
                modelChoices[index].downloadProgress = 0.0
            }
        }
        isDownloading = false
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 20) {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        VStack(spacing: 8) {
                            ForEach($viewModel.modelChoices) { $model in
                                OnboardingModelChoiceItemView(model: $model, viewModel: viewModel)
                            }
                        }
                    }
                }
...
struct OnboardingModelChoiceItemView: View {
    @Binding var model: OnboardingModelChoice
