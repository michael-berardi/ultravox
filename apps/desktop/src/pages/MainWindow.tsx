...
export function MainWindow({
  ...
  const reactiveVisualsEnabled = config?.reactive_visuals_enabled ?? false;
  useEffect(() => {
    if (!recording || !reactiveVisualsEnabled) {
      setInputLevel(0);
      return;
    }
    let cancelled = false;
    ...
    const tick = async () => {
      if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
        ...
        try {
          ...
          if (!cancelled) setInputLevel(level);
        } catch {
          if (!cancelled) setInputLevel(0);
        }
      }
      if (!cancelled) timer = window.setTimeout(() => void tick(), 80);
    };
    void tick();
    return () => {
      cancelled = true;
  ...
  const transcribingId = transcribing ? latestRecording?.id ?? null : null;
      ...
      <main ref={mainRef} className="main">
        {needsOnboarding ? (
          ...
          <section className="hero" aria-labelledby="activity-status">
            ...
            {reactiveVisualsEnabled && recording && (
              <RecordingMeter active level={inputLevel} />
            )}
...
const LATEST_CARD_TRANSCRIBING_DELAY_MS = 1_000;
...
function RecordingCard({
  ...
  const delayedInProgress = useDelayedVisibility(
    context === "latest" && isInProgress ? row.id : null,
    LATEST_CARD_TRANSCRIBING_DELAY_MS,
  );
  const showInProgress = context === "history" || delayedInProgress;
  const showStatus = context === "history"
    || (isInProgress ? showInProgress : row.status !== "completed");
    ...
    <div className={`model-card${selected ? " selected" : ""}`}>
      ...
      >
        ...
        <span className="model-card-copy">
          <strong>{title}</strong>
          <span className={downloaded ? "downloaded" : ""}>{size} · {status}</span>
          {preparing && (
            <progress
              value={fraction}
              max={1}
