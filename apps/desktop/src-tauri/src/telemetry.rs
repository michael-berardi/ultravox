...
const ENDPOINT: &str = "https://analytics.implosecybernetics.com/api/app-telemetry/event";
...
fn sanitize_loaded_state(state: &mut StoredState) -> bool {
    ...
    if invalid_in_flight {
        ...
        in_flight.counters.model_downloads_completed = in_flight
            .counters
...
impl Telemetry {
    pub fn new(app: AppHandle) -> Result<Self, String> {
        let directory = crate::state::data_dir(&app)?;
...
mod tests {
    ...
    fn pending_usage_keeps_local_buckets_until_successful_delivery() {
        ...
        assert_eq!(
            completed_usage(&state, "2026-08-16").1,
            UsageCounters::default()
        );
        assert_eq!(
