...
const USAGE: &str = "Usage: dictator-control [health|status|model-catalog|history-smoke|download-smoke|shortcut-config-smoke|audio-devices|live-record-smoke|recording-id-db-smoke|paste-bridge-dry-run|caret-bridge-dry-run|transcribe-fixture-smoke|transcribe <path> [v2|v3]|voice-health|voice-start|voice-stop <id>|voice-status <id>|voice-cancel <id>]";

fn data_dir() -> PathBuf {
    std::env::var("DICTATOR_DATA_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| std::env::temp_dir().join("dictator-control"))
}
...
async fn run_download_smoke() -> Result<(), String> {
    ...
    let body = b"dictator model smoke test data";
    ...
    let models_dir =
        std::env::temp_dir().join(format!("dictator-control-models-{}", Uuid::new_v4()));
...
fn run_shortcut_config_smoke() -> Result<(), String> {
    let dir = std::env::temp_dir().join(format!("dictator-control-shortcut-{}", Uuid::new_v4()));
...
async fn run_live_record_smoke() -> Result<(), String> {
    ...
    let mut config = dictator_core::AudioInputConfig::default();
...
fn run_paste_bridge_dry_run() -> Result<(), String> {
    let result = bridge::paste_text("Dictator paste bridge dry run");
    if result < 0 {
...
async fn run_voice_request(command: &str, recording_id: Option<&str>) -> Result<(), String> {
    ...
    let mut stream = UnixStream::connect(dictator_desktop_lib::voice_ipc::socket_path())
        .await
        .map_err(|e| e.to_string())?;
    stream
